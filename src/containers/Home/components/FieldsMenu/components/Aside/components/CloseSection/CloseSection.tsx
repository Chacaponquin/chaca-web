...
export default function CloseSection({ handleCloseFieldsMenu }: Props) {
  ...
  const iconClass = clsx("transition-all duration-200", "-translate-y-[1px]", "flex items-center", {
    "-translate-x-1": hover,
  })
  ...
  return (
    <section className="flex justify-start pt-4 pb-1">
      <button
      ...
      >
        ...
        <p className="text-base font-fontMedium">{CLOSE_MESSAGE}</p>
      </button>
    </section>
  )
}
