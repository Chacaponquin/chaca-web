...
const OptionsContainer = ({
  ...
  const [optionsWithOpenArguments, setOptionsWithOpenArguments] = useState<string[]>([])
  ...
  const handleCloseArguments = (id: string) => {
    setOptionsWithOpenArguments(optionsWithOpenArguments.filter((o) => o !== id))
  }
    ...
    <div className='flex flex-col gap-1 px-5'>
      {options.map((o, i) => (
        <Option
          option={o}
          key={o.id}
          args={findType(field.dataType.fieldType.parent, field.dataType.fieldType.type).arguments}
          field={field}
...
const Option = ({
  ...
  return (
    <div className={divClass()}>
      ...
      {openArguments && args.length > 0 && <OptionArguments args={args} field={field} />}
    </div>
  )
}
