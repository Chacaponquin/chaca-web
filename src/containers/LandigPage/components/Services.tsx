  ...
  return (
    <div className='w-screen flex justify-center items-center flex-col gap-5 lg:px-20 px-14 esm:px-5 lg:py-20 sm:py-12 esm:py-6'>
      ...
      <div className='grid lg:grid-cols-3 gap-6 w-full grid-cols-1'>
        {services.map((el, i) => (
          <div key={uuid()} className='shadow-lg rounded-md w-full text-center'>
            <img
              src={el.image}
              alt=''
            ...
            <div className='flex flex-col p-6 esm:px-3'>
              <h1 className='font-fontBold mb-2 text-2xl esm:text-xl'>{el.title}</h1>
              <p className='mb-0 text-gray-500 text-lg esm:text-base'>{el.description}</p>
            </div>
          </div>
        ))}
      </div>
